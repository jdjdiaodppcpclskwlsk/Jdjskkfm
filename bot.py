...
def podnyal():
    ...
    for admin in Own:
        k.execute("INSERT OR IGNORE INTO svoi VALUES (?)", (admin,))
    c.commit(); c.close()
...
def xod():
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="✖️ Отмена", callback_data="die")]])

def end_kb():
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="✔️ Отправить", callback_data="snd"), InlineKeyboardButton(text="◀️ Назад", callback_data="bk")]])
...
async def _a(m: Message):
    if not svoy(m.from_user.id):
        await m.answer("✖️ Нет прав."); return
    await m.answer("⚙️ Админ-панель:", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="📃 Заявления", callback_data="amenu")]]))
...
async def _q1(m: Message, state: FSMContext):
    ...
    await m.answer("⚖️ Какое наказание тебе выдали и за что:", reply_markup=xod())
...
async def _snd(c: CallbackQuery, state: FSMContext):
    ...
    await c.message.edit_text("✔️ Заявка отправлена", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="◀️ Назад", callback_data="gmain")]]))
...
async def _ml(c: CallbackQuery):
    ...
    if not rows:
        await c.message.edit_text("📭 У тебя нет заявлений.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="◀️ Назад", callback_data="gmain")]])); return
    btns = [[InlineKeyboardButton(text=f"📋 {z}", callback_data=f"vm_{n}")] for n, z in rows]
    btns.append([InlineKeyboardButton(text="◀️ Назад", callback_data="gmain")])
...
async def _vm(c: CallbackQuery):
    ...
    if not r: await c.answer("✖️ Не найдено"); return
    tt, pp, ss, ww, st = r
    smap = {"ojid": "⏳ На рассмотрении", "prinyat": "✔️ Оспорено", "otboi": "✖️ Отказано"}
    await c.message.edit_text(f"📄 Заявление\n\n<b>📝 Название:</b> {tt}\n<b>⚖️ Наказание:</b> {pp}\n<b>📌 Ситуация:</b> {ss}\n<b>🤔 Что хочу:</b> {ww}\n\n<b>📊 Статус:</b> {smap.get(st, st)}",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="◀️ Назад", callback_data="ml")]]), parse_mode="HTML")

@d.callback_query(F.data == "amenu")
async def _amenu(c: CallbackQuery):
    if not svoy(c.from_user.id): await c.answer("✖️ Нет прав."); return
    await c.message.edit_text("📃 Заявления:", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="✔️ Оспорены", callback_data="al_prinyat_0")],
        [InlineKeyboardButton(text="✖️ Отказано", callback_data="al_otboi_0")],
        [InlineKeyboardButton(text="◀️ Назад", callback_data="abk")]
    ]))

@d.callback_query(F.data == "abk")
async def _abk(c: CallbackQuery):
    await c.message.edit_text("⚙️ Админ-панель:", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="📃 Заявления", callback_data="amenu")]]))
...
async def _al(c: CallbackQuery):
    ...
    if not chunk:
        await c.message.edit_text("📭 Пусто.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="◀️ Назад", callback_data="amenu")]])); return
    ...
    btns.append([InlineKeyboardButton(text="◀️ Назад", callback_data="amenu")])
...
async def _av(c: CallbackQuery):
    ...
    btns = []
    if tip == "ojid":
        btns.append([InlineKeyboardButton(text="✔️ Оспорить", callback_data=f"yes_{idd}_{tip}_{pg}"), InlineKeyboardButton(text="✖️ Отказать", callback_data=f"no_{idd}_{tip}_{pg}")])
    btns.append([InlineKeyboardButton(text="◀️ Назад", callback_data=f"al_{tip}_{pg}")])
...
async def _yes(c: CallbackQuery):
    ...
    if own:
        try: await b.send_message(own[0], "✔️ Твоя заявка оспорена")
        except: pass
    await c.message.edit_text("✔️ Оспорено.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="◀️ Назад", callback_data=f"al_ojid_{pg}")]]))
...
async def _no(c: CallbackQuery):
    ...
    con.commit(); con.close()
    if own:
        try: await b.send_message(own[0], "✖️ Твоя заявка отклонена.")
        except: pass
    await c.message.edit_text("✖️ Отклонено.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="◀️ Назад", callback_data=f"al_ojid_{pg}")]]))
